...
from config import SPONSOR_TYPE_TO_TEMPLATE, EXCEL_COLUMNS, FILTER_BY_OUTREACH, OUTREACH_FILTER_VALUE
...
class SponsorReader:
    ...
    def get_sponsors(self) -> List[Dict]:
        ...
        if self.sponsor_type_column is None:
            # Try to use the configured column name first
        ...
        for _, row in self.df.iterrows():
            # Filter by "Assigned Team Member" if enabled
            ...
            sponsor = {
                ...
                "row_data": row.to_dict()  # Store all row data for additional placeholders
            }
